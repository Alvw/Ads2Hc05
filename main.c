...
void onRF_MultiByteMessage(){
  ...
  while (msgOffset < (rf_rx_buf[0]-2)){
    ...
    if(rf_rx_buf[msgOffset] == 0xF1){//«апись регистров ads1292
      AFE_Write_Reg(rf_rx_buf[msgOffset+1], rf_rx_buf[msgOffset+2], &rf_rx_buf[msgOffset+3]);
      msgOffset+=rf_rx_buf[msgOffset+2]+3;
    }
    if(rf_rx_buf[msgOffset] == 0xF2){//делители частоты дл€ ads1292 2 значени€
      for(int i = 0; i<2; i++){
    ...
    if(rf_rx_buf[msgOffset] == 0xF3){//–ежим работы акселерометра
